  ...
  template: `
    <div class="admin-page">
      ...
      <mat-card>
        ...
        <p *ngIf="appointments.length === 0 && !loading" class="empty-msg">No appointments found.</p>
        <p *ngIf="loading" class="empty-msg">Loading...</p>
      </mat-card>
    </div>
  `,
  styles: [`
    ...
    @media (max-width: 768px) { mat-card { overflow-x: auto; } }
  `]
})
export class SalonOwnerAppointmentsComponent implements OnInit {
  appointments: any[] = [];
  filterForm: FormGroup;
  loading = false;
  columns = ['customer', 'service', 'technician', 'datetime', 'status'];

  constructor(private mySalonService: MySalonService, private fb: FormBuilder, private snackBar: MatSnackBar) {
    this.filterForm = this.fb.group({ date: [''], status: [''] });
  }
  ...
  load() {
    const params: any = {};
    const { date, status } = this.filterForm.value;
    if (date) params['date'] = new Date(date).toISOString().split('T')[0];
    if (status) params['status'] = status;
    this.loading = true;
    this.mySalonService.getAppointments(params).subscribe({
  ...
  getService(a: any): any { return (a.serviceId as any) || {}; }
  getTech(a: any): any { return (a.technicianId as any) || {}; }

  updateStatus(a: any, status: string) {
    this.mySalonService.updateAppointment(a._id, { status }).subscribe({
      next: () => { a.status = status; this.snackBar.open('Status updated', 'OK', { duration: 2000 }); },
