...
@Component({
  selector: 'app-salon-owner-services',
  standalone: true,
  imports: [
  ...
  template: `
    <div class="admin-page">
      ...
      <mat-card *ngIf="showForm" class="form-card">
        <mat-card-header>
          <mat-card-title>{{editId ? 'Edit' : 'Add'}} Service</mat-card-title>
        </mat-card-header>
        <mat-card-content>
          <form [formGroup]="form" class="form-grid">
            <mat-form-field><mat-label>Name</mat-label><input matInput formControlName="name"></mat-form-field>
            <mat-form-field><mat-label>Price ($)</mat-label><input matInput formControlName="price" type="number" min="0"></mat-form-field>
            <mat-form-field><mat-label>Duration (min)</mat-label><input matInput formControlName="duration" type="number" min="15"></mat-form-field>
            <mat-form-field>
      ...
      <mat-card>
        <table mat-table [dataSource]="services" class="full-width">
          ...
          <ng-container matColumnDef="category">
            ...
            <td mat-cell *matCellDef="let s"><span class="badge-primary">{{s.category}}</span></td>
          </ng-container>
          ...
          <ng-container matColumnDef="actions">
            ...
            <td mat-cell *matCellDef="let s">
              <button mat-icon-button color="primary" (click)="edit(s)" matTooltip="Edit"><mat-icon>edit</mat-icon></button>
              <button mat-icon-button color="warn" (click)="delete(s._id)" matTooltip="Delete"><mat-icon>delete</mat-icon></button>
            </td>
          </ng-container>
          <tr mat-header-row *matHeaderRowDef="columns"></tr>
          <tr mat-row *matRowDef="let row; columns: columns;"></tr>
        </table>
        <p class="empty" *ngIf="services.length === 0 && !loading">No services yet. Add your first service!</p>
  ...
  styles: [`
    ...
    table { width: 100%; }
    .badge-primary { background: #e8f5e9; color: var(--primary-dark, #1b5e20); font-size: 0.78rem; padding: 2px 8px; border-radius: 50px; }
...
export class SalonOwnerServicesComponent implements OnInit {
  services: any[] = [];
  form: FormGroup;
  showForm = false;
  editId = '';
  ...
  constructor(private mySalonService: MySalonService, private fb: FormBuilder, private snackBar: MatSnackBar) {
    this.form = this.fb.group({
      ...
      category: ['Manicure', Validators.required],
      description: ['']
    });
  }
  ...
  load() {
    this.loading = true;
    this.mySalonService.getServices().subscribe({
  ...
  openForm() {
    this.showForm = true;
    this.editId = '';
    this.form.reset({ category: 'Manicure', price: 0, duration: 60 });
    this.imagePreview = '';
    this.imageFile = null;
  }
  ...
  edit(s: any) {
    this.showForm = true;
    this.editId = s._id;
    this.form.patchValue({ name: s.name, price: s.price, duration: s.duration, category: s.category, description: s.description });
  ...
  save() {
    ...
    const req = this.editId
      ? this.mySalonService.updateService(this.editId, fd)
      : this.mySalonService.createService(fd);
    req.subscribe({
  ...
  delete(id: string) {
    ...
    this.mySalonService.deleteService(id).subscribe({
