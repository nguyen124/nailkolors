...
@Component({
    ...
    <mat-sidenav-container class="layout-container">
      <mat-sidenav #sidenav [mode]="isMobile ? 'over' : 'side'" [opened]="!isMobile" class="sidenav">
        <div class="sidenav-header">
          ...
          <div class="user-info"><mat-icon>store</mat-icon><span>Salon Owner</span></div>
          <div class="owner-name">{{auth.currentUser()?.name}}</div>
          <div class="slug-badge" *ngIf="slug">/{{slug}}</div>
        </div>
        <mat-nav-list>
          <a mat-list-item [routerLink]="['/salon-owner', slug, 'dashboard']" routerLinkActive="active">
            <mat-icon matListItemIcon>dashboard</mat-icon><span matListItemTitle>Dashboard</span>
          </a>
          <a mat-list-item [routerLink]="['/salon-owner', slug, 'services']" routerLinkActive="active">
            <mat-icon matListItemIcon>spa</mat-icon><span matListItemTitle>Services</span>
          </a>
          <a mat-list-item [routerLink]="['/salon-owner', slug, 'colors']" routerLinkActive="active">
            <mat-icon matListItemIcon>palette</mat-icon><span matListItemTitle>Nail Colors</span>
          </a>
          <a mat-list-item [routerLink]="['/salon-owner', slug, 'technicians']" routerLinkActive="active">
            <mat-icon matListItemIcon>people</mat-icon><span matListItemTitle>Technicians</span>
          </a>
          <a mat-list-item [routerLink]="['/salon-owner', slug, 'appointments']" routerLinkActive="active">
            <mat-icon matListItemIcon>calendar_today</mat-icon><span matListItemTitle>Appointments</span>
          </a>
          <a mat-list-item [routerLink]="['/salon-owner', slug, 'posts']" routerLinkActive="active">
            <mat-icon matListItemIcon>article</mat-icon><span matListItemTitle>Blog / Promos</span>
          </a>
          <a mat-list-item [routerLink]="['/salon-owner', slug, 'profile']" routerLinkActive="active">
            <mat-icon matListItemIcon>store</mat-icon><span matListItemTitle>Salon Profile</span>
          </a>
        </mat-nav-list>
      ...
      <mat-sidenav-content class="main-content">
        <mat-toolbar class="toolbar">
          ...
          <span class="toolbar-title">{{salonService.profile()?.salonName || 'Salon Owner Portal'}}</span>
  ...
  styles: [`
    ...
    .sidenav-header { padding: 20px 16px; border-bottom: 1px solid rgba(255,255,255,0.1); }
    ...
    .user-info mat-icon { font-size: 16px; height: 16px; width: 16px; color: var(--primary-light); }
    .owner-name { font-size: 0.95rem; font-weight: 600; color: white; margin-top: 4px; }
    .slug-badge { font-size: 0.72rem; color: rgba(255,255,255,0.45); margin-top: 2px; font-family: monospace; }
...
export class SalonOwnerLayoutComponent implements OnInit {
  slug = '';
  isMobile = false;

  constructor(public auth: AuthService, public salonService: MySalonService, private route: ActivatedRoute) {
  ...
  ngOnInit() {
    ...
    this.salonService.loadProfile().subscribe();
  }
